...
<script lang="ts">
  ...
  const embedSnippet = $derived(
    `<a href="${hubUrl}"><img src="${badgeUrl}" alt="WhoEarns Live validator badge" width="440" height="76"></a>`,
  );

  // X intent. Body text deliberately compact — the OG card carries
  // the heavy formatting.
  const xMessage = $derived.by(() => {
    const name = display ?? `${vote.slice(0, 4)}…${vote.slice(-4)}`;
    const prefix = tierLabel ? `${name} · ${tierLabel}` : name;
...
<div class="flex items-center gap-1">
  <a
    ...
    class="inline-flex h-9 w-9 items-center justify-center rounded-lg text-[color:var(--color-text-subtle)] hover:bg-[color:var(--color-surface-muted)] hover:text-[color:var(--color-text-default)] transition-colors"
    title="Share on X"
