...
<script lang="ts">
  ...
  const QA = [
    ...
    },
    {
      ...
      a_ko: '세 가지 차이가 있습니다. (1) validator 수입을 leader slot당 값으로 정규화하고 indexed validator 평균과 비교해 피어 대비 잘하는지 못하는지 판단 가능; (2) MEV tips를 각 생산 블록에서 도출하므로 진행 중 에폭 수입도 거의 실시간으로 갱신 가능; (3) 모든 파생 데이터가 CC0이고 공개 API로 제공. 타 대시보드는 보통 로그인이나 벤더 엔드포인트 뒤에 둡니다.',
    },
    {
