...
// `public_ref` (migration 0042) is read on every SELECT so the route
// layer has the opaque `walletRef` to serve + resolve by. INSERT does
// NOT set it — the column DEFAULT (`gen_random_uuid()::text`)
// generates it server-side.
const COLS = `vote_pubkey, wallet_pubkey, public_ref, label, signed_nonce,
...
/**
 * The pre-insert payload for `OperatorWalletsRepository.insert` — an
 * `OperatorWallet` MINUS `publicRef`. `public_ref` (migration 0042)
 * is generated by the column DEFAULT (`gen_random_uuid()::text`)
 * server-side at INSERT time, so the caller cannot — and must not —
 * supply it. Every READ path returns the full `OperatorWallet`
 * (`publicRef` included); only this write payload omits it.
 */
export type OperatorWalletInsert = Omit<OperatorWallet, 'publicRef'>;
...
export class OperatorWalletsRepository {
   ...
   *
   * The payload omits `publicRef` (`OperatorWalletInsert`): the
   * `public_ref` column DEFAULT generates it server-side, so the
   * INSERT column list deliberately does not include it.
   */
  async insert(wallet: OperatorWalletInsert): Promise<OperatorWalletInsertResult> {
    try {
  ...
  /**
   * Resolve an opaque `public_ref` (migration 0042) back to its
   * ACTIVE (not-expired) `OperatorWallet` row for a given vote. The
   * unregister route uses this: the client only ever holds the
   * opaque ref, so the route looks the wallet up by `(vote_pubkey,
   * public_ref)` to recover the full pubkey it needs for the
   * `delete` call + the audit record.
   *
   * Returns `null` when no row matches OR the registration has
   * lapsed — same `expires_at > NOW()` gate as the other ACTIVE
   * reads, so a stale registration is treated as already-gone.
   * `public_ref` is UNIQUE; the implicit one-row-per-ref invariant
   * means no `LIMIT` is needed, but the gate can still drop the row
   * when it expired.
   */
  async findActiveByVoteAndRef(
    vote: VotePubkey,
    publicRef: string,
  ): Promise<OperatorWallet | null> {
    const { rows } = await this.pool.query<OperatorWalletRow>(
      `SELECT ${COLS} FROM operator_wallets
        WHERE vote_pubkey = $1
          AND public_ref = $2
