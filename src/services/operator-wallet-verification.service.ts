...
/**
 * Unregister-ceremony nonce.
 *
 * `walletRef` is the opaque per-registration token (the
 * `operator_wallets.public_ref` UUID — migration 0042), NOT the full
 * operator-wallet pubkey. Binding the REF lets the claim-page client
 * build this nonce from the `walletRef` it received on `GET
 * /v1/claims/:vote` without ever holding the full pubkey, and means
 * no server "challenge" round-trip is needed. The identity signature
 * still covers the whole canonical form, so the ref cannot be
 * re-bound to a different validator/identity.
 */
export interface OperatorWalletUnregisterNonce {
  purpose: typeof OPERATOR_WALLET_UNREGISTER_NONCE_PURPOSE;
  votePubkey: VotePubkey;
  identityPubkey: IdentityPubkey;
  /** Opaque registration ref — the `public_ref` token, not the pubkey. */
  walletRef: string;
  issuedAtMs: number;
  expiresAtMs: number;
  domain: string;
 ...
 * `walletRef` sorts last (after `votePubkey`). Mirror in
 * `ui/src/routes/claim/[vote]/+page.svelte`'s
 * `buildUnregisterNonceJson` — the two MUST stay byte-identical.
 */
...
/**
 * The successful-`verify` payload carries the wallet row to INSERT —
 * an `OperatorWallet` MINUS `publicRef`. The opaque `public_ref`
 * (migration 0042) is generated by the column DEFAULT at INSERT
 * time; this service runs strictly BEFORE the INSERT, so it cannot
 * know the ref. Structurally identical to the repo's
 * `OperatorWalletInsert` — `operatorWalletsRepo.insert(result.wallet)`
 * type-checks without a cast.
 */
export type VerifyOperatorWalletResult =
  | { ok: true; wallet: Omit<OperatorWallet, 'publicRef'> }
...
export class OperatorWalletVerificationService {
   ...
   *
   * The canonical nonce binds an opaque `walletRef` (the
   * `public_ref` token), not the wallet pubkey — this method only
   * Ed25519-verifies the identity signature over that canonical
   * form, it does NOT crypto-decode the wallet field, so the
   * field-shape change is transparent to the verification logic.
   * The route resolves `walletRef` -> the real pubkey separately.
   *
